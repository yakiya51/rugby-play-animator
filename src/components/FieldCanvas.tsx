...
export default function FieldCanvas() {
    ...
    const isSettingInitialPositions = useStore(
        (s) => s.isSettingInitialPositions,
    );
    ...
    const handlePlayerDragStart = useCallback((id: string) => {
        if (useStore.getState().isSettingInitialPositions) {
            useStore.getState().selectPlayer(id);
        } else {
    ...
    const handlePlayerDragMove = useCallback(
        (id: string, gx: number, gy: number) => {
            if (useStore.getState().isSettingInitialPositions) {
                useStore
                    .getState()
                    .setPlayerInitialPosition(id, gx - TRY_ZONE, gy);
            } else {
    ...
    return (
        <div ref={containerRef} className="relative w-full h-full bg-zinc-950">
            ...
            <Toolbar
                zoom={zoom}
                isVert={isVert}
                isSettingInitialPositions={isSettingInitialPositions}
                onZoomIn={() =>
                ...
                onToggleInitialPositions={() =>
                    useStore.getState().toggleSetInitialPositions()
                }
            />
        </div>
    );
            ...
            {players.map((p) => {
                ...
                return (
                    <Line
                        ...
                        opacity={
                            selected ? 0.85 : selectedPlayerId ? 0.25 : 0.5
                        }
                        dash={[3 * px, 3 * px]}
                        lineCap="round"
                        lineJoin="round"
...
function Toolbar({
    ...
    onReset: () => void;
    onToggle: () => void;
    onToggleInitialPositions: () => void;
}) {
    return (
        <div className="absolute top-2 right-2 flex items-center gap-1">
            <TooltipProvider delayDuration={300}>
                {isSettingInitialPositions && (
                    <span className="text-xs text-amber-400 font-semibold mr-1 select-none animate-pulse">
                        Setting Positions
                    </span>
                )}
                <TBtn
                    tip={
                        isSettingInitialPositions
                            ? "Exit Initial Position Mode"
                            : "Set Initial Positions"
                    }
                    onClick={onToggleInitialPositions}
                    active={isSettingInitialPositions}
                >
