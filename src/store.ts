...
interface State {
    ...
    isSettingInitialPositions: boolean;
    _past: Snapshot[];
    _future: Snapshot[];
    ...
    stopRecording: () => void;
    toggleSetInitialPositions: () => void;
    setPlayerInitialPosition: (id: string, x: number, y: number) => void;
    moveSegment: (
        trackId: string,
        segIndex: number,
        newStartTick: number,
...
export const useStore = create<State>((set, get) => ({
    ...
    isSettingInitialPositions: false,
    isPlaying: false,
    _past: [],
    _future: [],
        ...
        set({
            ...
            tracks: s.tracks.map((t) =>
                t.id === trackId
                    ? {
                          ...t,
                          segments: t.segments.filter((_, i) => i !== segIndex),
                      }
                    : t,
            ),
    ...
    toggleSetInitialPositions: () => {
        const s = get();
        const entering = !s.isSettingInitialPositions;
        if (entering) {
            // Stop any active recording/playback, jump to tick 0
            if (s.recordingPlayerId) get().stopRecording();
            set({
                isSettingInitialPositions: true,
                isPlaying: false,
                currentTick: 0,
            });
            get().applyKeyframesAtTick(0);
        } else {
            set({ isSettingInitialPositions: false });
        }
    },

    setPlayerInitialPosition: (id, x, y) => {
        const s = get();
        s.pushSnapshot();
        set({
            players: s.players.map((p) => (p.id === id ? { ...p, x, y } : p)),
    ...
    startRecording: (playerId) => {
        ...
        if (
            track?.segments.some(
                (seg) => tick >= seg.startTick && tick < seg.endTick,
            )
        )
            get().pushSnapshot();
        const kf: Keyframe = {
    ...
    moveSegment: (trackId, segIndex, newStartTick) => {
        ...
        for (const o of others) {
            if (ns < o.endTick && ns + dur > o.startTick) {
                ns =
                    newStartTick < seg.startTick
                        ? o.endTick
                        : o.startTick - dur;
            }
        }
        ns = Math.round(Math.max(0, Math.min(ns, s.totalTicks - dur)));
        if (others.some((o) => ns < o.endTick && ns + dur > o.startTick))
            return;
